...
export class InternshipController {
    ...
    try {
      ...
      if (!userId || !sessionId) {
        return this.c.json(
          createResponse(
            false,
            "Unauthorized: User ID or Session ID not found",
          ),
          401,
        );
      }

      const internshipData = await this.internshipService.getInternshipData(
        userId,
        sessionId,
      );
      ...
      return this.c.json(
        createResponse(
          true,
          "Internship data retrieved successfully",
          internshipData,
        ),
        200,
      );
      ...
      if (
        ...
        (error.message.includes("No active internship") ||
  ...
  checkInternshipStatus = async () => {
    try {
      ...
      const data = await this.internshipService.getInternshipData(
        userId,
        sessionId,
      );
      if (!data || !data.internship) {
        return this.c.json(
          createResponse(false, "No active internship found"),
          404,
        );
      }
      ...
      const isAssessmentFilled = await documentService.isAssessmentFilled(
        data.internship.id,
      );

      return this.c.json(
        createResponse(true, "Status checked", {
  ...
  generateDocument = async () => {
    try {
      ...
      if (type === "logbook") {
        ...
        const isFilled = await documentService.isAssessmentFilled(
          data.internship.id,
        );
      ...
      const buffer =
        type === "logbook"
          ? await documentService.generateLogbook(userId, sessionId, {
              format,
              withSignature,
            })
          : await documentService.generateAssessment(userId, sessionId, {
              format,
              withSignature,
            });
      ...
      this.c.header(
        "Content-Disposition",
        `attachment; filename="${type}-${userId}.${ext}"`,
      );
