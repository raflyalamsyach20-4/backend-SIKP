...
export class SuratPengantarDosenController {
  ...
  constructor(private c: Context<{ Bindings: CloudflareBindings }>) {
    this.suratPengantarDosenService = new SuratPengantarDosenService(
      this.c.env,
    );
  }
  ...
  getRequests = async () => {
    try {
      ...
      return this.c.json(
        createResponse(
          true,
          "Daftar pengajuan surat pengantar berhasil diambil",
          requests,
        ),
      );
    } catch (error) {
      return handleError(
        this.c,
        error,
        "Failed to get surat pengantar requests",
      );
    }
  };
  ...
  approve = async () => {
    try {
      ...
      const result = await this.suratPengantarDosenService.approveRequest(
        requestId,
        user.dosenId!,
        user.role,
        sessionId,
      ...
      return this.c.json(
        createResponse(
          true,
          "Pengajuan surat pengantar berhasil disetujui",
          result,
        ),
      );
    } catch (error) {
      return handleError(
        this.c,
        error,
        "Failed to approve surat pengantar request",
      );
    }
  };
  ...
  reject = async () => {
    try {
      ...
      return this.c.json(
        createResponse(
          true,
          "Pengajuan surat pengantar berhasil ditolak",
          result,
        ),
      );
    } catch (error) {
      ...
      return handleError(
        this.c,
        error,
        "Failed to reject surat pengantar request",
      );
    }
  };
}
