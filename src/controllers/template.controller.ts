...
export class TemplateController {
  ...
  async getAll() {
    try {
      ...
      return this.c.json(
        createResponse(true, "Templates berhasil diambil", templates),
        200,
      );
    } catch (error) {
  ...
  async getById() {
    try {
      ...
      if (!template) {
        return this.c.json(
          createResponse(false, "Template tidak ditemukan"),
          404,
        );
      }

      return this.c.json(
        createResponse(true, "Template berhasil diambil", template),
        200,
      );
    } catch (error) {
  ...
  async create() {
    try {
      ...
      if (user.role !== "admin") {
        return this.c.json(
          createResponse(false, "Anda tidak memiliki akses untuk endpoint ini"),
          403,
        );
      }
      ...
      const file =
        (fileEntry as any) instanceof File
      ...
      if (!name) {
        return this.c.json(
          createResponse(false, "Nama template wajib diisi"),
          400,
        );
      }
      ...
      return this.c.json(
        createResponse(true, "Template berhasil dibuat", result.template),
        201,
      );
    } catch (error) {
  ...
  async update() {
    try {
      ...
      if (result.error) {
        ...
        return this.c.json(
          createResponse(false, result.error || "Gagal mengupdate template"),
          400,
        );
      }

      return this.c.json(
        createResponse(true, "Template berhasil diupdate", result.template),
        200,
      );
    } catch (error) {
  ...
  async delete() {
    try {
      ...
      if (!result.success) {
        ...
        return this.c.json(
          createResponse(false, result.error || "Gagal menghapus template"),
          400,
        );
      }

      return this.c.json(
        createResponse(true, "Template berhasil dihapus"),
        200,
      );
    } catch (error) {
  ...
  async download() {
    try {
      ...
      const userRole =
        (this.c.get("user") as JWTPayload | undefined)?.role || "MAHASISWA";
      ...
      const contentDisposition = isPreview
        ? "inline"
        : `attachment; filename="${template.originalName}"`;
