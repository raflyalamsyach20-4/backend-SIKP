...
export class MentorWorkflowController {
  ...
  submitMentorApprovalRequest = async (validated: any) => {
    try {
      ...
      const request = await this.service.submitMentorApprovalRequest(
        studentUserId,
        validated,
      );
      return this.c.json(
        createResponse(true, "Mentor approval request submitted", request),
        201,
      );
    } catch (error) {
  ...
  getMyMentorRequest = async () => {
    try {
      ...
      const requests = await this.service.getMyMentorRequest(
        studentUserId,
        sessionId,
      );
      return this.c.json(
        createResponse(
          true,
          "Your mentor approval requests retrieved",
          requests,
        ),
        200,
      );
  ...
  listMentorApprovalRequests = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Mentor approval requests retrieved", requests),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  approveMentorApprovalRequest = async () => {
    try {
      ...
      const result = await this.service.approveMentorApprovalRequest(
        requestId,
        reviewerUserId,
        sessionId,
      );
      return this.c.json(
        createResponse(true, "Mentor approval request approved", result),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  rejectMentorApprovalRequest = async (validated: any) => {
    try {
      ...
      const request = await this.service.rejectMentorApprovalRequest(
        requestId,
        reviewerUserId,
        validated.reason,
      );
      return this.c.json(
        createResponse(true, "Mentor approval request rejected", request),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  resubmitMentorApprovalRequest = async (validated: any) => {
    try {
      ...
      const request = await this.service.resubmitMentorApprovalRequest(
        requestId,
        studentUserId,
        validated,
      );
      return this.c.json(
        createResponse(true, "Mentor approval request resubmitted", request),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  createMentorEmailChangeRequest = async (validated: any) => {
    try {
      ...
      const req = await this.service.createMentorEmailChangeRequest(
        mentorId,
        validated.requestedEmail,
        validated.reason,
      );
      return this.c.json(
        createResponse(true, "Email change request submitted", req),
        201,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  listMentorEmailChangeRequests = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Email change requests retrieved", reqs),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  approveMentorEmailChangeRequest = async () => {
    try {
      ...
      const result = await this.service.approveMentorEmailChangeRequest(
        requestId,
        reviewerUserId,
      );
      return this.c.json(
        createResponse(true, "Email change request approved", result),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  rejectMentorEmailChangeRequest = async (validated: any) => {
    try {
      ...
      const result = await this.service.rejectMentorEmailChangeRequest(
        requestId,
        reviewerUserId,
        validated.reason,
      );
      return this.c.json(
        createResponse(true, "Email change request rejected", result),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  getDosenLogbookMonitor = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Logbook monitor data retrieved", data),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  getDosenLogbookMonitorByStudent = async () => {
    try {
      ...
      const data =
        await this.service.getDosenLogbookMonitorByStudent(studentUserId);
      return this.c.json(
        createResponse(true, "Student logbook monitor data retrieved", data),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  joinLeaderMentor = async () => {
    try {
      const studentUserId = this.getUserId();
      if (!studentUserId)
        return this.c.json(createResponse(false, "Unauthorized"), 401);

      const result = await this.service.joinLeaderMentor(studentUserId);
      return this.c.json(
        createResponse(
          true,
          "Successfully joined team leader's mentor",
          result,
        ),
        200,
      );
