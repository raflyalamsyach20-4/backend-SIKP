...
export class PenilaianController {
  ...
  getKriteria = async () => {
    try {
      const type = (this.c.req.query("type") || "MENTOR") as
        | "MENTOR"
        | "DOSEN_PA";
      const kriteria =
        type === "DOSEN_PA"
          ? DEFAULT_DOSEN_PA_KRITERIA
          : DEFAULT_MENTOR_KRITERIA;
      ...
      if (stored.length > 0) {
        ...
        return this.c.json(
          createResponse(
            true,
            `Penilaian kriteria (${type}) retrieved successfully`,
            {
              kriteria: mapped,
              totalWeight: mapped.reduce(
                (sum: number, k: any) => sum + k.weight,
                0,
              ),
  ...
  getRecap = async () => {
    try {
      ...
      const data = await this.assessmentService.getAssessmentRecap(
        internshipId,
        sessionId,
      );
      return this.c.json(
        createResponse(true, "Assessment recap retrieved", data),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  printRecap = async () => {
    try {
      ...
      const sessionId = user.sessionId!;
      const internshipId = this.c.req.param("internshipId");
      const buffer = await this.assessmentService.generateGradeRecapPDF(
        internshipId,
        sessionId,
      );
      ...
      this.c.header(
        "Content-Disposition",
        `attachment; filename="Rekap-Nilai-${internshipId}.pdf"`,
      );
  ...
  /**
   * GET /api/penilaian/print-lecturer/:internshipId
   */
  printLecturerAssessment = async () => {
    try {
      const user = this.c.get("user") as JWTPayload;
      const sessionId = user.sessionId!;
      const internshipId = this.c.req.param("internshipId");
      const buffer = await this.assessmentService.generateLecturerAssessmentPDF(
        internshipId,
        sessionId,
      );
      ...
      this.c.header(
        "Content-Disposition",
        `attachment; filename="Penilaian-Dosen-${internshipId}.pdf"`,
      );
  ...
  updateKriteria = async () => {
    try {
      ...
      if (!Array.isArray(kriteria) || kriteria.length === 0) {
        return this.c.json(
          createResponse(false, "'kriteria' must be a non-empty array"),
          400,
        );
      }

      const totalWeight = kriteria.reduce(
        (sum: number, k: any) => sum + (Number(k.weight) || 0),
        0,
      );
      if (totalWeight !== 100) {
        return this.c.json(
      ...
      return this.c.json(
        createResponse(true, "Criteria updated", {
          kriteria,
          totalWeight,
          type,
        }),
  ...
  getPendingVerifications = async () => {
    try {
      ...
      if (!profileId) {
        return this.c.json(
          createResponse(false, "Profile ID not found in token"),
          401,
        );
      }

      const prodiName = typeof user.prodi === "string" ? user.prodi : null;
      const data = await this.assessmentService.getPendingVerifications(
        profileId,
        sessionId,
        prodiName,
      );
      return this.c.json(
        createResponse(true, "Pending verifications retrieved", data),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  /**
   * GET /api/penilaian/kaprodi/pending-lecturer
   */
  getPendingLecturerVerifications = async () => {
    try {
      const user = this.c.get("user") as JWTPayload;
      const profileId = user.profileId;
      const sessionId = user.sessionId!;
      ...
      const prodiName = typeof user.prodi === "string" ? user.prodi : null;
      const data = await this.assessmentService.getPendingLecturerVerifications(
        profileId,
        sessionId,
        prodiName,
      );
      return this.c.json(
        createResponse(true, "Pending lecturer verifications retrieved", data),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  verifyGrade = async () => {
    try {
      ...
      if (!profileId || !gradeId) {
        return this.c.json(
          createResponse(false, "Missing profileId or gradeId"),
          400,
        );
      }

      const result = await this.assessmentService.verifyGrade(
        gradeId,
        profileId,
        sessionId,
      );
      return this.c.json(
        createResponse(true, "Grade verified successfully", result),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  /**
   * POST /api/penilaian/kaprodi/verify-lecturer/:assessmentId
   */
  verifyLecturer = async () => {
    try {
      const user = this.c.get("user") as JWTPayload;
      const profileId = user.profileId;
      const sessionId = user.sessionId!;
      ...
      if (!profileId || !assessmentId) {
        return this.c.json(
          createResponse(false, "Missing profileId or assessmentId"),
          400,
        );
      }

      const result = await this.assessmentService.verifyLecturerAssessment(
        assessmentId,
        profileId,
        sessionId,
      );
      return this.c.json(
        createResponse(
          true,
          "Lecturer assessment verified successfully",
          result,
        ),
        200,
      );
  ...
  getAdminPendingVerifications = async () => {
    try {
      ...
      const results =
        await this.assessmentService.getPendingAdminVerifications(sessionId);
      return this.c.json(
        createResponse(true, "Pending admin verifications retrieved", results),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
  ...
  verifyGradeByAdmin = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Grade verified by admin successfully", result),
        200,
      );
    } catch (error) {
      return handleError(this.c, error);
