...
export class MahasiswaController {
  ...
  dashboard = async () => {
    try {
      ...
      const dashboardData = await this.mahasiswaService.getDashboard(
        user.mahasiswaId,
        sessionId,
      );

      return this.c.json(
        createResponse(true, "Mahasiswa dashboard retrieved", dashboardData),
      );
    } catch (error) {
      return handleError(
        this.c,
        error,
        "Failed to retrieve mahasiswa dashboard",
      );
    }
  };
  ...
  search = async () => {
    try {
      ...
      if (!prodiId || !fakultasId) {
        console.warn(
          `[MahasiswaController.search] User ${user.userId} missing prodiId (${prodiId}) or fakultasId (${fakultasId})`,
        );
      }
      ...
      if (!ssoResponse.success || !ssoResponse.data) {
        return this.c.json(
          createResponse(false, "Gagal mencari mahasiswa dari SSO", []),
        );
      }
      ...
      return this.c.json(
        createResponse(
          true,
          "Mahasiswa search results retrieved",
          mappedResults,
        ),
      );
    } catch (error) {
