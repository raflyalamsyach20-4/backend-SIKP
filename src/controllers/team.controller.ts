...
export class TeamController {
  ...
  createTeam = async () => {
    try {
      ...
      console.log(
        `[TeamController.createTeam] Request from mahasiswaId=${user.mahasiswaId}, profileId=${user.profileId}, dosenPAId=${user.dosenPAId}`,
      );

      if (!user.mahasiswaId) {
        throw new Error(
          "mahasiswaId not found in session - SSO profile incomplete or student identity not selected",
        );
      }
      ...
      return this.c.json(
        createResponse(true, "Team created successfully", team),
        201,
      );
    } catch (error) {
  ...
  inviteMember = async () => {
    try {
      ...
      if (!user.mahasiswaId) {
        throw new Error(
          "mahasiswaId not found in session - student identity required",
        );
      }
      ...
      return this.c.json(
        createResponse(true, "Invitation sent successfully", invitation),
        201,
      );
    } catch (error) {
  ...
  respondToInvitation = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Invitation response recorded", result),
      );
    } catch (error) {
  ...
  getTeamMembers = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Team members retrieved", members),
      );
    } catch (error) {
  ...
  leaveTeam = async () => {
    try {
      ...
      if (!user.mahasiswaId)
        throw new Error("mahasiswaId not found in session");
      console.log(
        `[TeamController.leaveTeam] Request from mahasiswaId=${user.mahasiswaId}, teamId=${teamId}`,
      );
      ...
      return this.c.json(
        createResponse(true, "Successfully left the team", result),
      );
    } catch (error) {
  ...
  removeMember = async () => {
    try {
      ...
      if (!user.mahasiswaId)
        throw new Error("mahasiswaId not found in session");
      console.log(
        `[TeamController.removeMember] Request from mahasiswaId=${user.mahasiswaId}, teamId=${teamId}, memberId=${memberId}`,
      );
      ...
      return this.c.json(
        createResponse(true, "Member removed successfully", result),
      );
    } catch (error) {
  ...
  deleteTeam = async () => {
    try {
      ...
      if (!user.mahasiswaId)
        throw new Error("mahasiswaId not found in session");
      const result = await this.teamService.deleteTeam(
        teamId,
        user.mahasiswaId,
      );

      return this.c.json(
        createResponse(true, "Team deleted successfully", result),
      );
    } catch (error) {
  ...
  finalizeTeam = async () => {
    try {
      ...
      if (!user.mahasiswaId)
        throw new Error("mahasiswaId not found in session");
      console.log(
        `[TeamController.finalizeTeam] Request from mahasiswaId=${user.mahasiswaId}, teamId=${teamId}`,
      );

      const result = await this.teamService.finalizeTeam(
        teamId,
        user.mahasiswaId,
      );
      ...
      return this.c.json(
        createResponse(true, "Tim berhasil difinalisasi", result),
        200,
      );
    } catch (error) {
      ...
      return this.c.json(
        createResponse(
          false,
          appError.message || "Failed to finalize team",
          null,
        ),
        statusCode,
      );
  ...
  getMyInvitations = async () => {
    try {
      ...
      if (!user.mahasiswaId)
        throw new Error("mahasiswaId not found in session");
      const invitations = await this.teamService.getMyInvitations(
        user.mahasiswaId,
        this.c.get("sessionId"),
      );

      return this.c.json(
        createResponse(true, "Invitations retrieved", invitations),
      );
    } catch (error) {
  ...
  cancelInvitation = async () => {
    try {
      ...
      if (!user.mahasiswaId)
        throw new Error("mahasiswaId not found in session");
      console.log(
        `[TeamController.cancelInvitation] Request from mahasiswaId=${user.mahasiswaId}, memberId=${memberId}`,
      );

      const result = await this.teamService.cancelInvitation(
        memberId,
        user.mahasiswaId,
      );

      console.log(
        `[TeamController.cancelInvitation] ✅ Success: Invitation cancelled`,
      );
      return this.c.json(
        createResponse(true, "Invitation cancelled successfully", result),
      );
    } catch (error) {
  ...
  joinTeam = async () => {
    try {
      ...
      if (!user.mahasiswaId)
        throw new Error("mahasiswaId not found in session");
      console.log(
        `[TeamController.joinTeam] Request from mahasiswaId=${user.mahasiswaId}, teamCode=${teamCode}`,
      );

      const result = await this.teamService.joinTeam(
        teamCode,
        user.mahasiswaId,
        this.c.get("sessionId"),
      );
  ...
  resetTeam = async () => {
    try {
      ...
      if (!user.mahasiswaId)
        throw new Error("mahasiswaId not found in session");
      console.log(
        `[TeamController.resetTeam] Request from mahasiswaId=${user.mahasiswaId}`,
      );

      const myTeams = await this.teamService.getMyTeams(
        user.mahasiswaId,
        sessionId,
      );
      ...
      console.log(
        `[TeamController.resetTeam] Found team: ${team.code} (${team.id})`,
      );

      const result = await this.teamResetService.resetTeamByTeamId(team.id);

      console.log(
        `[TeamController.resetTeam] ✅ Success: Team reset completed`,
      );
      return this.c.json(
        createResponse(
          true,
          "Tim berhasil direset. Anda dapat membuat submission baru.",
          result,
        ),
      );
    } catch (error) {
