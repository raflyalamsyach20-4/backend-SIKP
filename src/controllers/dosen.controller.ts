...
export class DosenController {
  ...
  dashboard = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Dosen dashboard retrieved", dashboard),
      );
    } catch (error) {
  ...
  wakdekDashboard = async () => {
    try {
      ...
      const dashboard = await this.dosenService.getWakdekDashboard(
        user.dosenId!,
        user.role,
        sessionId,
      );
      ...
      return this.c.json(
        createResponse(true, "Wakil dekan dashboard retrieved", dashboard),
      );
    } catch (error) {
      return handleError(
        this.c,
        error,
        "Failed to retrieve wakil dekan dashboard",
      );
    }
  };
  ...
  me = async () => {
    try {
      ...
      const profile = await this.dosenService.getDosenById(
        user.dosenId!,
        sessionId,
      );
      ...
      if (!profile) {
        return this.c.json(
          createResponse(false, "Dosen profile not found in SSO"),
          404,
        );
      }
