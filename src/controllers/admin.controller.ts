...
export class AdminController {
  ...
  getDashboard = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Admin dashboard retrieved", dashboard),
      );
    } catch (error) {
          ...
          ? {
              ...
              firstDocument: first.documents?.[0]
                ? {
                    ...
                    uploadedByUser:
                      (first.documents![0] as any).uploadedByUser ?? null,
                  }
                : null,
            }
          : null,
  ...
  getSubmissionsByStatus = async () => {
    try {
      const status = this.c.req.param("status") as
        | "DRAFT"
        | "PENDING_REVIEW"
        | "REJECTED"
        | "APPROVED";
      const sessionId = this.c.get("sessionId");
      const submissions = await this.adminService.getSubmissionsByStatus(
        status,
        sessionId,
      );
      return this.c.json(
  ...
  getSubmissionById = async () => {
    try {
      ...
      const sessionId = this.c.get("sessionId");
      const submission = await this.adminService.getSubmissionById(
        submissionId,
        sessionId,
      );
      return this.c.json(
  ...
  approveSubmission = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Submission approved successfully", submission),
      );
    } catch (error) {
  ...
  rejectSubmission = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Submission rejected", submission),
      );
    } catch (error) {
  ...
  generateLetter = async () => {
    try {
      ...
      return this.c.json(
        createResponse(true, "Letter generated successfully", letter),
        201,
      );
    } catch (error) {
