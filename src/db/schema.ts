...
export const responseLetterStatusEnum = pgEnum("response_letter_status", [
  "pending",
  "submitted",
  "verified",
]);
export const suratKesediaanStatusEnum = pgEnum("surat_kesediaan_status", [
  "MENUNGGU",
  "DISETUJUI",
  "DITOLAK",
]);
export const suratPermohonanStatusEnum = pgEnum("surat_permohonan_status", [
  "MENUNGGU",
  "DISETUJUI",
  "DITOLAK",
]);
export const submissionVerificationStatusEnum = pgEnum(
...
export const logbookStatusEnum = pgEnum("logbook_status", [
  "PENDING",
  "APPROVED",
  "REJECTED",
]);
...
export const assessmentCriteriaTypeEnum = pgEnum("assessment_criteria_type", [
  "MENTOR",
  "DOSEN_PA",
]);
...
export const authSessions = pgTable(
  ...
  (table) => {
    return {
      idxAuthSessionsAuthUserId: index("idx_auth_sessions_auth_user_id").on(
        table.authUserId,
      ),
      idxAuthSessionsExpiresAt: index("idx_auth_sessions_expires_at").on(
        table.expiresAt,
      ),
    };
  },
...
export const submissions = pgTable(
  "submissions",
  {
    ...
    adminVerificationStatus: submissionVerificationStatusEnum(
    ...
    dosenVerificationStatus: submissionVerificationStatusEnum(
      "dosen_verification_status",
    )
      .notNull()
      .default("PENDING"),
    ...
    responseLetterStatus: responseLetterStatusEnum(
      "response_letter_status",
    ).default("pending"),
  ...
  (table) => {
    return {
      idxWorkflowStage: index("idx_submissions_workflow_stage").on(
        table.workflowStage,
      ),
      idxAdminVerificationStatus: index("idx_submissions_admin_status").on(
        table.adminVerificationStatus,
      ),
      idxDosenVerificationStatus: index("idx_submissions_dosen_status").on(
        table.dosenVerificationStatus,
      ),
      idxDosenQueue: index("idx_submissions_dosen_queue").on(
        table.workflowStage,
        table.dosenVerifiedByDosenId,
        table.createdAt,
      ),
      uqLetterNumber: uniqueIndex("submissions_letter_number_unique").on(
        table.letterNumber,
      ),
    };
  },
...
export const submissionDocuments = pgTable(
  ...
  (table) => {
    return {
      uqDocPerMember: uniqueIndex("uq_document_per_member").on(
        table.submissionId,
        table.documentType,
        table.memberMahasiswaId,
      ),
      idxSubmissionStatus: index("idx_submission_status").on(
        table.submissionId,
        table.status,
      ),
...
export const responseLetters = pgTable(
  "response_letters",
  {
    id: text("id").primaryKey(),
  ...
  (table) => {
    return {
      idxSubmissionId: index("idx_response_letters_submission_id").on(
        table.submissionId,
      ),
...
export const suratKesediaanRequests = pgTable(
  ...
  (table) => {
    return {
      uqRequest: uniqueIndex("uq_surat_kesediaan_request").on(
        table.memberMahasiswaId,
        table.dosenId,
      ),
      idxDosenStatus: index("idx_surat_kesediaan_dosen_status").on(
        table.dosenId,
        table.status,
      ),
      idxMemberStatus: index("idx_surat_kesediaan_member_status").on(
        table.memberMahasiswaId,
        table.status,
      ),
...
export const suratPermohonanRequests = pgTable(
  "surat_permohonan_requests",
  {
    ...
    mahasiswaEsignatureSnapshotAt: timestamp(
      "mahasiswa_esignature_snapshot_at",
    ),
    signedFileUrl: text("signed_file_url"),
    signedFileKey: text("signed_file_key"),
  ...
  (table) => {
    return {
      idxDosenStatusRequestedAt: index("idx_permohonan_dosen").on(
        table.dosenId,
        table.status,
        table.requestedAt,
      ),
      idxRequestedAt: index("idx_surat_permohonan_requested_at").on(
        table.requestedAt,
      ),
      idxMemberDosenStatus: index(
        "idx_surat_permohonan_member_dosen_status",
      ).on(table.memberMahasiswaId, table.dosenId, table.status),
    };
  },
);
...
export const lecturerAssessments = pgTable("lecturer_assessments", {
  ...
  isVerifiedByKaprodi: boolean("is_verified_by_kaprodi")
    .notNull()
    .default(false),
...
export const assessmentCriteria = pgTable(
  ...
  (table) => {
    return {
      idxAssessmentCriteriaType: index("idx_assessment_criteria_type").on(
        table.type,
      ),
      idxAssessmentCriteriaActive: index("idx_assessment_criteria_active").on(
        table.isActive,
      ),
      idxAssessmentCriteriaCategory: index(
        "idx_assessment_criteria_category",
      ).on(table.type, table.categoryId),
    };
  },
);
...
export const combinedGrades = pgTable("combined_grades", {
  ...
  lecturerAssessmentId: text("lecturer_assessment_id").references(
    () => lecturerAssessments.id,
    { onDelete: "set null" },
  ),
...
export const titleRevisions = pgTable("title_revisions", {
  id: text("id").primaryKey(),
  titleSubmissionId: text("title_submission_id")
    .notNull()
    .references(() => titleSubmissions.id, { onDelete: "cascade" }),
...
export const mentorEmailChangeRequests = pgTable(
  "mentor_email_change_requests",
  {
    id: text("id").primaryKey(),
...
export const responseLettersRelations = relations(
  responseLetters,
  ({ one }) => ({
...
export const generatedLettersRelations = relations(
  generatedLetters,
  ({ one }) => ({
    submission: one(submissions, {
...
export const suratPermohonanRequestsRelations = relations(
  suratPermohonanRequests,
  ({ one }) => ({
    submission: one(submissions, {
...
export const lecturerAssessmentsRelations = relations(
  lecturerAssessments,
  ({ one }) => ({
    internship: one(internships, {
...
export const titleSubmissionsRelations = relations(
  titleSubmissions,
  ({ one, many }) => ({
    internship: one(internships, {
