...
export class TemplateService {
      ...
      if (updates.file) {
        ...
        if (template.fileName) {
          try {
            ...
            console.error(
              "Failed to delete old template file from R2:",
              deleteError,
            );
            // Continue anyway
          }
    ...
    try {
      ...
      if (!fileData) {
        return {
          buffer: null,
          template,
          error: "File template tidak ditemukan di storage",
        };
      }
  ...
  private validateFile(file: File): { error?: string } {
    ...
    if (!allowedTypes.includes(file.type)) {
      return {
        error:
          "Tipe file tidak diizinkan. File yang diizinkan: .doc, .docx, .pdf, .html, .txt",
      };
    }
    ...
    if (!allowedExtensions.includes(extension.toLowerCase())) {
      return {
        error:
          "Ekstensi file tidak diizinkan. File yang diizinkan: .doc, .docx, .pdf, .html, .txt",
      };
    }
