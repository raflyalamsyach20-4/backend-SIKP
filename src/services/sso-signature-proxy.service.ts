...
export class SsoSignatureProxyService {
  ...
  async getActiveSignature(
    ...
    try {
      ...
      console.log(
        `[SsoSignatureProxyService.getActiveSignature] Fetching from SSO: ${url}`,
      );
      ...
      console.log(
        `[SsoSignatureProxyService.getActiveSignature] Response status: ${response.status}`,
      );
      ...
      if (!response.ok) {
        ...
        if (isLegacyError) {
          ...
          if (fallbackUrl && fallbackUrl !== url) {
            console.warn(
              `[SsoSignatureProxyService.getActiveSignature] Signature URL (${url}) failed. Trying fallback: ${fallbackUrl}`,
            );
        ...
        console.error(
          `[SsoSignatureProxyService.getActiveSignature] Failed with status ${response.status}. Response: ${responseText.substring(0, 200)}`,
        );
        return null;
      }
      ...
      console.log(
        `[SsoSignatureProxyService.getActiveSignature] Success. Active signature: ${payload.data?.activeSignature?.signatureId || "NONE"}`,
      );
      return payload.data.activeSignature;
    } catch (error) {
      console.error(
