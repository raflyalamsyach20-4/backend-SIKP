...
export class TeamService {
  ...
  async createTeam(mahasiswaId: string, dosenPAId: string | null | undefined) {
    console.log(
      `[createTeam] 🚀 Starting team creation for mahasiswaId=${mahasiswaId}, dosenPAId=${dosenPAId}`,
    );
    ...
    try {
      ...
      if (!dosenPAId) {
        console.error(
          `[createTeam] ❌ Missing dosenPAId for mahasiswaId=${mahasiswaId}`,
        );
        const err = new Error(
          "Dosen PA tidak ditemukan. Hubungi administrator untuk mengatur dosen PA.",
        ) as Error & {
          statusCode?: number;
        };
        err.statusCode = 400;
      ...
      const existingTeamsAsLeader =
        await this.teamRepo.findByLeaderMahasiswaId(mahasiswaId);
      if (existingTeamsAsLeader.length > 0) {
        console.error(
          `[createTeam] ❌ mahasiswaId already has ${existingTeamsAsLeader.length} team(s)`,
        );
        throw new Error(
          "You already have a team. Each student can only create one team",
        );
      }

      // Check if mahasiswaId is already a member of another team (ACCEPTED status)
      const existingMemberships =
        await this.teamRepo.findMembershipByMahasiswaId(mahasiswaId);
      const acceptedMembership = existingMemberships.find(
        (m) => m.invitationStatus === "ACCEPTED",
      );
      if (acceptedMembership) {
        console.error(
          `[createTeam] ❌ mahasiswaId already member of team: ${acceptedMembership.teamId}`,
        );
        throw new Error(
          "You are already a member of another team. Each student can only join one team",
        );
      }
      ...
      try {
        ...
        console.error(
          `[createTeam] ❌ Failed to add leader as member:`,
          memberError,
        );
  ...
  async leaveTeam(teamId: string, mahasiswaId: string) {
    console.log(
      `[leaveTeam] Processing leave: teamId=${teamId}, mahasiswaId=${mahasiswaId}`,
    );
    ...
    if (team.leaderMahasiswaId === mahasiswaId) {
      console.error(
        `[leaveTeam] ❌ Leader cannot leave team, must delete instead: ${teamId}`,
      );
      const err: Error = new Error(
        "Team leader cannot leave the team. Please delete the team instead.",
      );
      err.statusCode = 403;
      throw err;
    }
    ...
    if (!member) {
      console.error(
        `[leaveTeam] ❌ Member not found: mahasiswaId=${mahasiswaId}, teamId=${teamId}`,
      );
    ...
    // Verify deletion
    const verifyMember = await this.teamRepo.findMemberByTeamAndMahasiswa(
      teamId,
      mahasiswaId,
    );
    if (verifyMember) {
      console.error(
        `[leaveTeam] ⚠️ VERIFICATION FAILED: Member still exists after deletion!`,
      );
      throw new Error("Failed to remove member from team - please try again");
    }
    console.log(
      `[leaveTeam] ✅ VERIFICATION: Member successfully deleted from database`,
    );
  ...
  async removeMember(teamId: string, memberId: string, mahasiswaId: string) {
    console.log(
      `[removeMember] Processing removal: teamId=${teamId}, memberId=${memberId}, mahasiswaId=${mahasiswaId}`,
    );
    ...
    if (team.leaderMahasiswaId !== mahasiswaId) {
      console.error(
        `[removeMember] ❌ Unauthorized: User ${mahasiswaId} is not leader of team ${teamId}`,
      );
    ...
    if (team.status === "FIXED") {
      console.error(
        `[removeMember] ❌ Cannot remove member from finalized team: ${teamId}`,
      );
      const err: Error = new Error(
        "Cannot remove members from a finalized team",
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    if (
      member.role === "KETUA" ||
      member.mahasiswaId === team.leaderMahasiswaId
    ) {
    ...
    if (verifyMember) {
      console.error(
        `[removeMember] ⚠️ VERIFICATION FAILED: Member still exists after deletion!`,
      );
      throw new Error("Failed to remove member from team - please try again");
    }
    console.log(
      `[removeMember] ✅ VERIFICATION: Member successfully deleted from database`,
    );
  ...
  async deleteTeam(teamId: string, mahasiswaId: string) {
    console.log(
      `[deleteTeam] Processing deletion: teamId=${teamId}, mahasiswaId=${mahasiswaId}`,
    );
    ...
    if (team.leaderMahasiswaId !== mahasiswaId) {
      console.error(
        `[deleteTeam] ❌ Unauthorized: User ${mahasiswaId} is not leader of team ${teamId}`,
      );
    ...
    console.log(
      `[deleteTeam] Deleting team and cascade deleting team_members...`,
    );
  ...
  async finalizeTeam(teamId: string, mahasiswaId: string) {
    console.log(
      `[finalizeTeam] Processing finalization: teamId=${teamId}, mahasiswaId=${mahasiswaId}`,
    );
    ...
    if (team.leaderMahasiswaId !== mahasiswaId) {
      console.error(
        `[finalizeTeam] ❌ Unauthorized: User ${mahasiswaId} is not leader of team ${teamId}`,
      );
    ...
    if (alreadyFixed) {
      console.warn(
        `[finalizeTeam] ⚠️ Team already finalized, continue idempotently: ${teamId}`,
      );
    }
    ...
    const acceptedMembers = members.filter(
      (m) => m.invitationStatus === "ACCEPTED" && m.role !== "KETUA",
    );

    console.log(
      `[finalizeTeam] Team has ${members.length} total members, ${acceptedMembers.length} accepted (excluding leader)`,
    );
    console.log(
      `[finalizeTeam] Allowing finalization even with just the leader`,
    );

    // 4. ✅ Verify team has dosenKpId (should be set during creation from SSO profile)
    if (!team.dosenKpId) {
      console.error(
        `[finalizeTeam] ❌ Team missing dosenKpId (should have been set during creation)`,
      );
      const err: Error = new Error(
        "Dosen PA ketua belum ditetapkan. Hubungi administrator.",
      );
      err.statusCode = 422;
      throw err;
    }
    ...
    if (!alreadyFixed) {
      console.log(
        `[finalizeTeam] Updating team status to FIXED with existing dosenKpId...`,
      );
      updatedTeam = await this.teamRepo.update(teamId, {
        status: "FIXED",
      });
      console.log("[finalizeTeam] TEAM_FINALIZED", {
        teamId,
        dosenKpId: team.dosenKpId,
      });
    }
    ...
    if (!submission) {
      const defaults = this.buildDefaultDraftPayload(
        updatedTeam?.code || team.code,
      );
      ...
      if (!submission) {
        throw new Error(
          "Failed to create draft submission after team finalization",
        );
      }

      submissionAlreadyExists = !createdByThisRequest;
      if (createdByThisRequest) {
        console.log("[finalizeTeam] SUBMISSION_CREATED", {
          teamId,
  ...
  async inviteMember(
    ...
    if (team.status === "FIXED") {
      throw new Error(
        "Cannot invite members to a finalized team. Please create a new team if you want to add more members.",
      );
    }

    // Find member by NIM from SSO
    console.log(`[inviteMember] 🔍 Looking up student by NIM: ${memberNim}`);
    const member = await this.mahasiswaService.getMahasiswaByNim(
      memberNim,
      sessionId,
    );
    if (!member) {
      throw new Error(
        `Mahasiswa dengan NIM ${memberNim} tidak ditemukan di SSO`,
      );
    }

    // Check if member already has THIS team
    const existingMember = await this.teamRepo.findMemberByTeamAndMahasiswa(
      teamId,
      member.id,
    );
    if (existingMember) {
      ...
      if (status === "REJECTED") {
        ...
        console.log(
          `[inviteMember] 🗑️ Removing old REJECTED invitation: memberId=${existingMember.id}`,
        );
    ...
    console.log(
      `[inviteMember] Creating invitation: teamId=${teamId}, mahasiswaId=${member.id}, invitedBy=${mahasiswaId}`,
    );
  ...
  async respondToInvitation(
    ...
    console.log(
      `[respondToInvitation] 🔍 Processing: memberId=${memberId}, mahasiswaId=${mahasiswaId}, accept=${accept}`,
    );
    ...
    if (!memberRecord) {
      console.error(
        `[respondToInvitation] ❌ Member record not found: memberId=${memberId}`,
      );
    ...
    if (memberRecord.invitationStatus !== "PENDING") {
      console.error(
        `[respondToInvitation] ❌ Invalid status: ${memberRecord.invitationStatus} (expected PENDING)`,
      );
      const invalidStatusError: Error = new Error(
        "Invitation not found or already responded",
      );
    ...
    if (!team) {
      console.error(
        `[respondToInvitation] ❌ Team not found: ${memberRecord.teamId}`,
      );
      throw new Error("Team not found");
    }
    ...
    if (!isBeingInvited && !isTeamLeader) {
      console.error(
        `[respondToInvitation] ❌ Unauthorized: User is neither invitee nor team leader`,
      );
      const unauthorizedError: Error = new Error(
        "Unauthorized: only team leader or invitee can respond",
      );
    ...
    console.log(
      `[respondToInvitation] ✅ Valid authorization: teamId=${memberRecord.teamId}`,
    );
    ...
    if (accept && isBeingInvited) {
      console.log(
        `[respondToInvitation] Checking for existing teams as leader...`,
      );
      ...
      if (existingTeamsAsLeader.length > 0) {
        console.log(
          `[respondToInvitation] Found ${existingTeamsAsLeader.length} existing team(s) as leader`,
        );
        // Auto-delete old teams where user is leader
        for (const oldTeam of existingTeamsAsLeader) {
          try {
            console.log(
              `[respondToInvitation] Auto-deleting old team: ${oldTeam.id}`,
            );
            await this.teamRepo.deleteTeam(oldTeam.id);
            console.log(
              `[respondToInvitation] ✅ Old team deleted: ${oldTeam.id}`,
            );
          } catch (error) {
            console.error(
              `[respondToInvitation] ⚠️ Error deleting old team ${oldTeam.id}:`,
              error,
            );
    ...
    const updatedMember = await this.teamRepo.updateMemberStatus(
      memberRecord.id,
      status,
    );
    ...
    console.log(
      `[respondToInvitation] ✅ Successfully responded to invitation with status: ${status}`,
    );
      ...
      members.map(async (member) => {
        ...
        return {
          ...
          user: {
            ...
            email:
              student?.profile.emails.find((e) => e.isPrimary)?.email || "",
          },
        };
      }),
    );
  ...
  async getMyTeams(mahasiswaId: string, sessionId: string) {
    ...
    const userMemberships =
      await this.teamRepo.findMembershipByMahasiswaId(mahasiswaId);
    console.log(
      `[getMyTeams] Found ${userMemberships.length} total memberships`,
    );

    // Filter only ACCEPTED memberships
    const acceptedMemberships = userMemberships.filter(
      (m) => m.invitationStatus === "ACCEPTED",
    );
    console.log(
      `[getMyTeams] Found ${acceptedMemberships.length} ACCEPTED memberships`,
    );
    ...
    const teamsWithMembers = await Promise.all(
      acceptedMemberships.map(async (userMembership) => {
        ...
        if (!team) {
          console.warn(
            `[getMyTeams] ⚠️ Team not found: ${userMembership.teamId}`,
          );
          return null;
        }
    ...
    console.log(
      `[getMyTeams] Returning ${validTeams.length} teams with members`,
    );
    return validTeams;
  }
  ...
  async getMyInvitations(mahasiswaId: string, sessionId: string) {
    console.log(
      `[getMyInvitations] Fetching invitations for user: ${mahasiswaId}`,
    );

    const invitations =
      await this.teamRepo.findInvitationsByMahasiswaId(mahasiswaId);
    console.log(
      `[getMyInvitations] Found ${invitations.length} invitations for user: ${mahasiswaId}`,
    );
    ...
    const enrichedInvitations = await Promise.all(
      invitations.map(async (inv) => {
        ...
        if (!team) {
          console.warn(
            `[getMyInvitations] ⚠️ Team not found for invitation ${inv.id}`,
          );
          return null;
        }
        ...
        const leaderData = leader
          ? {
              ...
              email:
                leader.profile.emails.find((e) => e.isPrimary)?.email || "",
            }
          : null;
        ...
        if (inv.invitedByMahasiswaId) {
          const inviter = await this.mahasiswaService.getMahasiswaById(
            inv.invitedByMahasiswaId,
            sessionId,
          );
          if (inviter) {
            inviterData = {
              ...
              email:
                inviter.profile.emails.find((e) => e.isPrimary)?.email || "",
            };
          }
        }
  ...
  async cancelInvitation(memberId: string, leaderId: string) {
    console.log(
      `[cancelInvitation] Processing cancellation: memberId=${memberId}, leaderId=${leaderId}`,
    );
    ...
    if (!team) {
      console.error(
        `[cancelInvitation] ❌ Team not found: ${targetInvitation.teamId}`,
      );
      throw new Error("Team not found");
    }
    ...
    if (team.leaderMahasiswaId !== leaderId) {
      console.error(
        `[cancelInvitation] ❌ Unauthorized: User ${leaderId} is not leader of team ${team.id}`,
      );
    ...
    if (targetInvitation.invitationStatus !== "PENDING") {
      console.error(
        `[cancelInvitation] ❌ Cannot cancel non-PENDING invitation: status=${targetInvitation.invitationStatus}`,
      );
      const err: Error = new Error(
        `Cannot cancel ${targetInvitation.invitationStatus.toLowerCase()} invitation`,
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    // Cannot cancel KETUA
    if (
      targetInvitation.role === "KETUA" ||
      targetInvitation.mahasiswaId === team.leaderMahasiswaId
    ) {
      console.error(
        `[cancelInvitation] ❌ Cannot cancel KETUA invitation: ${memberId}`,
      );
  ...
  async joinTeam(teamCode: string, mahasiswaId: string, sessionId: string) {
    console.log(
      `[joinTeam] Processing join request: teamCode=${teamCode}, mahasiswaId=${mahasiswaId}`,
    );
    ...
    if (team.leaderMahasiswaId === mahasiswaId) {
      const err: Error = new Error(
        "Anda adalah ketua tim ini. Tidak dapat mengirim permintaan bergabung pada tim sendiri",
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    if (existingMember && existingMember.invitationStatus === "PENDING") {
      const err: Error = new Error(
        "Anda sudah mengirim permintaan bergabung ke tim ini. Tunggu persetujuan dari ketua tim",
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    if (otherTeamMembership) {
      const err: Error = new Error(
        "Anda masih menjadi anggota tim lain. Silakan keluar atau hapus tim lama terlebih dahulu",
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    if (acceptedMembers.length >= 3) {
      const err: Error = new Error(
        "Tim ini sudah memiliki jumlah anggota maksimal (3 anggota)",
      );
      err.statusCode = 400;
      throw err;
    }
    ...
    const teamLeader = await this.mahasiswaService.getMahasiswaById(
      team.leaderMahasiswaId,
      sessionId,
    );
