...
import {
  ...
  CreateLogbookData,
  UpdateLogbookData,
} from "@/repositories/logbook.repository";
...
export class LogbookService {
  ...
  async createLogbook(
    ...
    if (!internshipId) {
      throw this.createServiceError(
        "No active internship found. You must have an active internship to create logbook entries.",
  ...
  async updateLogbook(
    ...
    if (entry.status !== "PENDING" && entry.status !== "REJECTED") {
      throw this.createServiceError(
        `Cannot edit a logbook entry with status '${entry.status}'. Only PENDING or REJECTED entries can be modified.`,
        "LOGBOOK_NOT_MODIFIABLE",
        400,
      );
    }
  ...
  async deleteLogbook(userId: string, logbookId: string) {
    ...
    if (entry.status !== "PENDING" && entry.status !== "REJECTED") {
      throw this.createServiceError(
        `Cannot delete a logbook entry with status '${entry.status}'. Only PENDING or REJECTED entries can be deleted.`,
        "LOGBOOK_NOT_MODIFIABLE",
        400,
      );
    }
  ...
  async submitLogbook(userId: string, logbookId: string) {
    ...
    if (entry.status !== "PENDING") {
      throw this.createServiceError(
        `Logbook entry status is '${entry.status}'. Only PENDING entries can be submitted.`,
        "LOGBOOK_NOT_PENDING",
        400,
      );
    }
  ...
  async uploadPhoto(userId: string, logbookId: string, file: File) {
    ...
    if (entry.status !== "PENDING" && entry.status !== "REJECTED") {
      throw this.createServiceError(
        `Cannot upload photo to a logbook entry with status '${entry.status}'. Only PENDING or REJECTED entries can be modified.`,
        "LOGBOOK_NOT_MODIFIABLE",
        400,
      );
    }
    ...
    if (entry.fileName) {
      try {
        ...
        console.warn(
          "⚠️ [LogbookService] Failed to delete old attachment from storage:",
          err,
        );
      }
    }
    ...
    return {
      ...updated,
      photoUrl: updated.fileUrl
        ? this.storageService.getAssetProxyUrl(updated.fileUrl)
        : null,
    };
  }
}
