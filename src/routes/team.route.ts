...
export const createTeamRoutes = () => {
  const team = new Hono<{ Bindings: CloudflareBindings }>()
    ...
    .get("/my-teams", zValidator("query", emptyQuerySchema), async (c) =>
      new TeamController(c).getMyTeams(),
    )
    .get("/my-invitations", zValidator("query", emptyQuerySchema), async (c) =>
      new TeamController(c).getMyInvitations(),
    )
    .post(
      "/:teamId/invite",
    ...
    .post("/reset", zValidator("query", emptyQuerySchema), async (c) =>
      new TeamController(c).resetTeam(),
    );
