...
export const createResponseLetterRoutes = () => {
  const router = new Hono<{ Bindings: CloudflareBindings }>()
     ...
     */
    .post("/", zValidator("form", submitResponseLetterSchema), async (c) => {
     ...
     */
    .get("/admin", zValidator("query", emptyQuerySchema), async (c) => {
    ...
    .get(
      ...
      async (c) => {
        return new ResponseLetterController(
          c,
        ).getResponseLetterBySubmissionId();
      },
    )
     ...
     * Auth: Required
     */
    .get("/my", zValidator("query", emptyQuerySchema), async (c) => {
