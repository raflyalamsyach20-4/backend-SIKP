...
export const createMentorshipRoutes = () => {
  ...
  mentorship.post(
    ...
    (c) =>
      new MentorWorkflowController(c).submitMentorApprovalRequest(
        c.req.valid("json"),
      ),
  );

  mentorship.get("/requests/me", (c) =>
    new MentorWorkflowController(c).getMyMentorRequest(),
  );
  mentorship.post("/requests/join-leader", (c) =>
    new MentorWorkflowController(c).joinLeaderMentor(),
  );
  ...
  mentorship.post(
    ...
    (c) =>
      new MentorWorkflowController(c).rejectMentorApprovalRequest(
        c.req.valid("json"),
      ),
  );
  mentorship.post(
    ...
    (c) =>
      new MentorWorkflowController(c).resubmitMentorApprovalRequest(
        c.req.valid("json"),
      ),
  );
  ...
  mentorship.post("/profile/signature", (c) =>
    new MentorController(c).updateSignature(),
  );

  // --- Mentees Management (Mentor) ---
  mentorship.get("/mentees", mentorOnly, (c) =>
    new MentorController(c).getMentees(),
  );
  mentorship.get("/mentees/:studentId", mentorOnly, (c) =>
    new MentorController(c).getMenteeById(),
  );
  mentorship.get("/mentees/:studentId/logbooks", mentorOnly, (c) =>
    new MentorController(c).getStudentLogbooks(),
  );

  mentorship.post("/logbooks/:logbookId/approve", mentorOnly, (c) =>
    new MentorController(c).approveLogbook(),
  );
  mentorship.post(
    "/logbooks/:logbookId/reject",
    mentorOnly,
    zValidator("json", z.object({ rejectionReason: z.string() })),
    (c) => {
      const validated = c.req.valid("json");
      return new MentorController(c).rejectLogbook(validated);
    },
  );
  mentorship.post("/mentees/:studentId/approve-all", mentorOnly, (c) =>
    new MentorController(c).approveAllLogbooks(),
  );
  ...
  mentorship.get("/assessments/me", (c) =>
    new MentorController(c).getAssessmentForMe(),
  );
  mentorship.get("/assessments/:studentId", (c) =>
    new MentorController(c).getAssessmentByStudent(),
  );
  mentorship.put(
  ...
  mentorship.post("/assessments/:assessmentId/unlock", mentorOnly, (c) =>
