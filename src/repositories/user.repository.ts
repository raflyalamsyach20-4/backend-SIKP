...
import type { SsoEnvelope, SsoProfile } from '@/types';
...
export class UserRepository {
  private readonly profileCacheByProfileId = new Map<
    string,
    { profile: SsoProfile; expiresAt: number }
  >();
  private readonly profileCacheByToken = new Map<
    string,
    { profile: SsoProfile | null; expiresAt: number }
  >();
  ...
  private getPrimaryEmail(profile: SsoProfile): string | null {
    const primary = profile.emails.find((item) => item.isPrimary)?.email;
    return primary || profile.emails[0]?.email || null;
  }

  private mapProfileRole(profile: SsoProfile): BasicUser['role'] {
  ...
  private toBasicUserFromProfile(profile: SsoProfile): BasicUser {
    return {
      id: profile.id,
  ...
  private toMahasiswaProfileFromSso(profile: SsoProfile): MahasiswaProfile {
  ...
  private toDosenProfileFromSso(profile: SsoProfile): DosenProfile {
